...
def compute_stencil2d(coords, model, dx, return_mixed_derivs=False):
    ...
    posx = coords[:, 0:1] + paddle.ones_like(coords[:, 0:1]) * dx
    negx = coords[:, 0:1] - paddle.ones_like(coords[:, 0:1]) * dx
    posy = coords[:, 1:2] + paddle.ones_like(coords[:, 0:1]) * dx
    negy = coords[:, 1:2] - paddle.ones_like(coords[:, 0:1]) * dx

    uposx = model(paddle.concat([posx, coords[:, 1:2], coords[:, 2:3]], axis=1))
    unegx = model(paddle.concat([negx, coords[:, 1:2], coords[:, 2:3]], axis=1))
    uposy = model(paddle.concat([coords[:, 0:1], posy, coords[:, 2:3]], axis=1))
    unegy = model(paddle.concat([coords[:, 0:1], negy, coords[:, 2:3]], axis=1))

    if return_mixed_derivs:
        uposxposy = model(paddle.concat([posx, posy, coords[:, 2:3]], axis=1))
        uposxnegy = model(paddle.concat([posx, negy, coords[:, 2:3]], axis=1))
        unegxposy = model(paddle.concat([negx, posy, coords[:, 2:3]], axis=1))
        unegxnegy = model(paddle.concat([negx, negy, coords[:, 2:3]], axis=1))
...
def compute_stencil3d(coords, model, dx, return_mixed_derivs=False):
    ...
    posy = coords[:, 1:2] + paddle.ones_like(coords[:, 0:1]) * dx
    negy = coords[:, 1:2] - paddle.ones_like(coords[:, 0:1]) * dx
    posz = coords[:, 2:3] + paddle.ones_like(coords[:, 0:1]) * dx
    negz = coords[:, 2:3] - paddle.ones_like(coords[:, 0:1]) * dx

    uposx = model(paddle.concat([posx, coords[:, 1:2], coords[:, 2:3]], axis=1))
    unegx = model(paddle.concat([negx, coords[:, 1:2], coords[:, 2:3]], axis=1))
    uposy = model(paddle.concat([coords[:, 0:1], posy, coords[:, 2:3]], axis=1))
    unegy = model(paddle.concat([coords[:, 0:1], negy, coords[:, 2:3]], axis=1))
    uposz = model(paddle.concat([coords[:, 0:1], coords[:, 1:2], posz], axis=1))
    unegz = model(paddle.concat([coords[:, 0:1], coords[:, 1:2], negz], axis=1))
    ...
    if return_mixed_derivs:
        ...
        uposxposz = model(paddle.concat([posx, coords[:, 1:2], posz], axis=1))
        uposxnegz = model(paddle.concat([posx, coords[:, 1:2], negz], axis=1))
        unegxposz = model(paddle.concat([negx, coords[:, 1:2], posz], axis=1))
        unegxnegz = model(paddle.concat([negx, coords[:, 1:2], negz], axis=1))

        uposyposz = model(paddle.concat([coords[:, 0:1], posy, posz], axis=1))
        uposynegz = model(paddle.concat([coords[:, 0:1], posy, negz], axis=1))
        unegyposz = model(paddle.concat([coords[:, 0:1], negy, posz], axis=1))
        unegynegz = model(paddle.concat([coords[:, 0:1], negy, negz], axis=1))
...
def compute_connectivity_tensor(nodes, edges):
    ...
    for k, v in node_edges.items():
        if len(v) < max_connectivity:
            ...
            v = v + empty_list
            node_edges[k] = paddle.to_tensor(v)
            ...
            node_edges[k] = paddle.to_tensor(v)
    connectivity_tensor = (
        paddle.stack([v for v in node_edges.values()], axis=0)
        .to(paddle.int64)
        .to(nodes.place)
    )
...
class GradientsAutoDiff(paddle.nn.Layer):
    """
...
class GradientsMeshlessFiniteDifference(paddle.nn.Layer):
    """
...
class GradientsFiniteDifference(paddle.nn.Layer):
    """
...
class GradientsSpectral(paddle.nn.Layer):
    """
    ...
    def forward(self, input_dict):
        ...
        u_h = paddle.fft.fftn(u, axes=list(range(2, self.dim + 2)))
        ...
        for i, nx in enumerate(n):
            kx.append(
                paddle.concat(
                    (
                        paddle.arange(start=0, end=nx // 2, step=1).to(device=u.place),
                        paddle.arange(start=-nx // 2, end=0, step=1).to(device=u.place),
                    ),
                    0,
        ...
        j = paddle.complex(
            paddle.to_tensor([0.0], place=u.place),
            paddle.to_tensor([1.0], place=u.place),
        )
        ...
        if self.order == 1:
            ...
            wx = paddle.concat(
                [
                    paddle.fft.ifftn(wx_h_i, axes=list(range(2, self.dim + 2))).real()
                    for wx_h_i in wx_h
                ],
                axis=1,
            )
            ...
            wxx = paddle.concat(
                [
                    paddle.fft.ifftn(wxx_h_i, axes=list(range(2, self.dim + 2))).real()
            ...
            if self.return_mixed_derivs:
                ...
                w_xy = paddle.fft.ifftn(
                    w_xy_h, axes=list(range(2, self.dim + 2))
                ).real()
                ...
                if self.dim == 3:
                    ...
                    w_xz = paddle.fft.ifftn(
                        w_xz_h, axes=list(range(2, self.dim + 2))
                    ).real()
                    ...
                    w_yz = paddle.fft.ifftn(
                        w_yz_h, axes=list(range(2, self.dim + 2))
                    ).real()
...
class GradientsLeastSquares(paddle.nn.Layer):
    """
...
class GradientCalculator:
    ...
    >>> coords = paddle.rand(10, 3).requires_grad_(True)
    ...
    def __init__(self, device: Optional[str] = None):
        self.place = device if device is not None else paddle.place("cpu")
        self.methods = {}
