...
class FirstDerivSecondOrder(paddle.nn.Layer):
    ...
    def __init__(self, dim: int, dx: Union[float, List[float]]):
        ...
        self.register_buffer("ddx1D", paddle.to_tensor([-1.0 / 2.0, 0.0, 1.0 / 2.0]))

    def get_convolution_kernel(self, axis):
        shape = [1, 1] + axis * [1] + [-1] + (self.dim - axis - 1) * [1]
        # multiply with 1/dx
        return (1 / self.dx[axis]) * paddle.reshape(self.ddx1D, shape)

    def apply_convolution(self, u, kernel, axis):
        if self.dim == 1:
            conv_result = paddle.nn.functional.conv1d(
                u, kernel, stride=1, padding=0, bias=None
            )
        elif self.dim == 2:
            conv_result = paddle.nn.functional.conv2d(
                u, kernel, stride=1, padding=0, bias=None
            )
        elif self.dim == 3:
            conv_result = paddle.nn.functional.conv3d(
                u, kernel, stride=1, padding=0, bias=None
            )
    ...
    def forward(self, u) -> List[Tensor]:
        u = paddle.nn.functional.pad(u, self.dim * (1, 1), "replicate")
        result = []
...
class SecondDerivSecondOrder(paddle.nn.Layer):
    ...
    def __init__(self, dim: int, dx: Union[float, List[float]]):
        ...
        self.register_buffer("d2dx21D", paddle.to_tensor([1.0, -2.0, 1.0]))

    def get_convolution_kernel(self, axis):
        shape = [1, 1] + axis * [1] + [-1] + (self.dim - axis - 1) * [1]
        # multiply with 1/dx**2
        return (1 / (self.dx[axis] ** 2)) * paddle.reshape(self.d2dx21D, shape)
...
class MixedSecondDerivSecondOrder(paddle.nn.Layer):
    ...
    def __init__(self, dim: int, dx: Union[float, List[float]]):
        ...
        self.register_buffer("ddx1D", paddle.to_tensor([-1.0, 0.0, 1.0]))

    def get_convolution_kernel(self, axis):
        shape = [1, 1] + axis * [1] + [-1] + (self.dim - axis - 1) * [1]
        return paddle.reshape(self.ddx1D, shape)
    ...
    def forward(self, u) -> List[Tensor]:
        ...
        u_pad = paddle.nn.functional.pad(u, pad, "constant")

        u_xplus = u_pad[:, :, :-2]
        u_xminus = u_pad[:, :, 2:]
        u_xplus = paddle.nn.functional.pad(u_xplus, self.dim * (1, 1), "replicate")
        u_xminus = paddle.nn.functional.pad(u_xminus, self.dim * (1, 1), "replicate")
        ...
        if self.dim == 3:  # compute dydz
            ...
            u_yplus = u_pad[:, :, :, :-2]
            u_yminus = u_pad[:, :, :, 2:]
            u_yplus = paddle.nn.functional.pad(u_yplus, self.dim * (1, 1), "replicate")
            u_yminus = paddle.nn.functional.pad(
                u_yminus, self.dim * (1, 1), "replicate"
            )
